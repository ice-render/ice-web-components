...
- **peer 下限抬到 `ice-render@^2.18.0`**：那一版是**事件系统改版** —— 事件沿组件树冒泡
  （命中组件 → 各级父容器 → 总线）、`ICEEvent` 的 W3C 方法真生效、事件名类型化。
  本仓已经按新语义收口：① 引擎事件名改用 `ICE_EVENT_NAME_CONSTS` 常量；
  ② 四处「容器点击」加了 `evt.target === this` 守卫（模态遮罩 / 悬浮按钮 / 图片预览遮罩 / 下拉选择器）
  —— 这条守卫**在新旧引擎上都成立**，所以本仓对 2.17.0 的行为不变。

  仍停在 2.14~2.17 的宿主装本版本会得到 npm 的 peer 冲突提示 —— 有意的：
  事件语义变了却继续用老引擎，会出现「点对话框把模态关了」「点工具栏按钮把预览关了」
  或「preventDefault 抛异常」这类只在运行时才炸的差异。

### 修复

- **`preventDefault` / `stopPropagation` 在旧引擎上会抛异常**：本仓有几处对 **ICEEvent**
  调用它们（`ICEKeyScope`、`ICETable` 的 Tab 拦截、`ICEModal` / `ICEDrawer` / `ICETour` 的面板点击）——
  2.17.0 里这两个方法是 `throw new Error('Method not implemented.')` 的桩，`typeof === 'function'`
  的守卫拦不住它们。2.18.0 起它们是真实现，这些调用才开始按预期生效。
## [1.21.0] - 2026-09-19
...
- **peer 下限抬到 `ice-render@^2.17.0`**：那一版把 `zIndex` 的默认值改成 **`'auto'` 哨兵**
  ...
  （`bringToFront` / `sendToBack` / `moveUp` / `moveDown`），并把**序列化格式升到 v2**
  （旧文档载入时自动迁移，绘制次序逐项不变）。本包内部按 zIndex 抬升子件（卡片 extra /
  滚动条 / 吸顶条 / 弹层…）的地方本来就取 `Number(...) || 0`，与这套口径一致。
...
- **`'auto'` 默认值下"取数静默变 NaN"**：`ICEHoverManager` 的兜底排序原来是 `state.zIndex || 0`
  —— `'auto'` 走 `||` 仍是字符串，相减得 NaN，排序**静默失效**（看着就是不生效）。改成
  `Number(...) || 0`；`ICEScrollPane` / `ICECard` / `ICEAffix` 三条断言同样先折数字再比。
- 模态测试里"遮罩在场景组件之上"改成断言**层归属**（遮罩挂在工具层，工具层整体在组件层之上），
  不再断言"创建得更晚 → zIndex 更高"（那条代理关系属于旧的计数器语义）。

## [1.20.0] - 2026-09-18

### 变更

- **peer 下限抬到 `ice-render@^2.16.0`**：本包的画布/表单/控件最终由引擎渲染，2.16.0 修的是「连线横穿图面里的其他图元」；顺带把宿主引擎基线抬到同一版。
...
- `ICEForm` 类文档里那句 `stretch` 的作用边界**收短成一句话**，细节留在
  `docs/guides/layout.md`。原因不是"写得越短越好"，而是**类文档的第一段会被下游
  `ice-web-components-dsl` 的组件清单摘成一条摘要**，那条摘要有 500 字上限
  （`tests/catalog.test.ts` 守着）—— 上游类文档越长，下游清单里那条就越像一整节文档。
  契约本身没变，仍是 1.19.2 那条：`stretch` 拉的是表单项，不是表单项里的控件。
