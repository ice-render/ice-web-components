...
> 掌机改成只保留一条状态线。`qa:arcade` 39 → 46 项。
>
> 第十四批补丁（2048 的两个真 bug）：
> ① **自绘动画不逐帧重画** —— `ICETileMap` 内部改动只置了组件自己的 `dirty`，而引擎渲染循环看的是
> `ice.dirty`，于是 `pulse` 的白色方块**卡在格子上不动**（用户报「小方块外层的盒子也在动」）。
> 现在所有内部改动走 `requestPaint()`（组件 + ice 双置脏），并补了单测与 QA（脉冲必须亮起再淡出）。
> ② 2048 的高亮从「所有变动的格子」收窄成**只闪真正合并的格子**（模型新增 `getLastMerged()`，2 条单测）。
> 顺带把外壳布局按 20px 栅格对齐：卡带行四等宽按钮、HUD 卡片等距、进度条与标签贴边、
> 底部按钮等宽、2048 棋盘在屏幕框里垂直居中。
>
...
## 阶段 E：业务覆盖补强（2026-09-12 评估 → 已开工）

### 评估结论

以 `875f191` 为基线清点：**77 个组件源文件 / 85 个组件类 / 94 个运行时导出 / 10 个模型 /
23 个工具函数与主题常量**（分组：基础 13、数据录入 15、录入（浮层）8、数据展示 20、
反馈与状态 15、导航 7、核心与布局 8）。

两个完整案例（六页后台 admin、客服工作台 workbench）证明：**中等复杂度的中后台与工具类界面，
覆盖率大约七到八成** —— 表单（含异步校验、跨字段依赖）、表格（分页/多选/排序/空态）、
列表树、浮层体系、反馈、导航、布局这一整套都是通的。

差距不在「组件数量」，而在**深度**，按「能不能上生产」排序如下。这一阶段就按这个顺序推。

| 优先级 | 缺口 | 现状（证据） | 目标 | 验收标准 |
|---|---|---|---|---|
| **P0** | **文本编辑态与 IME** | `ICETextField` 自己手写 `keydown` 追加字符 → **中文输入法打不进去**，只能 `setValue()`；引擎的 `ICEText.startEditing()` 早有透明 `<input>` + `compositionend` | 组件库自己也走「原生输入替身」：聚焦时在组件上方挂一个透明 input，`input` / `compositionend` 回写 value，`change` 与表单取值契约不变；密码框、文本域同样受益 | 单测覆盖挂载/卸载/回写/最大长度/Enter/Escape/无 document 降级；浏览器 QA 用 `insertText` 真打中文并断言值 |
| **P1** | **大数据量** | 表格只有分页，列表/树全量渲染；没有虚拟滚动 → 上千行开始卡，上万行不能上 | 虚拟滚动（`ICEVirtualList` 或给 `ICETable`/`ICEList`/`ICETree` 加 `virtual`）＋ 表格列宽拖动与固定列 | 1 万行数据滚动时**只渲染可视区**（断言节点数上界）、滚动位置与命中正确；QA 打开 1 万行示例页验证 |
| **P1** | **拖拽编辑** | 全库没有拖拽排序；表格/树/看板都只能靠按钮上下移 | 表格行拖拽排序、树节点拖拽（含跨层级落点提示）、看板卡片拖拽 | 单测覆盖「移动语义」（before/after/inside 三种落点）；QA 真鼠标拖拽并断言数据顺序变化 |
| **P1** | **可访问性与国际化** | 引擎有 `getAccessibilityTree()` 但组件库一行没接（读屏为空）；文案硬编码中文、无 locale 包 | 焦点/激活/值变化时把语义信息写进引擎的 a11y 树；提供 `iceI18n`（locale 包 + 运行时切换）并让内置文案走它 | 单测断言关键控件在 a11y 树里有 role/label/value；QA 切换语言后断言界面文案变化 |
| — | 图表 / 流程图 / 富文本 | 按既定分工：图表与流程图**归独立工程**，不在本库做；富文本编辑器（内容可编辑）在 canvas 里成本极高、收益有限，同样不做 | ⊘ 不做（不是遗漏，是边界） | — |

### 本阶段进度

- [x] **P0 文本编辑态与 IME**（`875f191` → 本阶段第一个提交）：
      `src/util/ICENativeInput.ts` 原生输入替身（透明 input/textarea、定位、聚焦、光标、
      `input` + `compositionend` 回写、`maxLength`、Enter/Escape/blur、无 document 降级）；
      `ICETextField` / `ICEPasswordField` / `ICETextArea` 聚焦挂载、失焦卸载，同值不重复
      `change`，单行回车抛 `submit`。踩到并修掉两个真坑：① **mousedown 的默认焦点转移**会把
      刚挂上的替身打掉（改成「松开鼠标后延迟一拍再挂」）；② **DOM 层假失焦**会连带组件失焦
      （改成「组件仍聚焦就把焦点抢回来」）。19 条单测 + `qa:admin` 4 条真中文输入断言。
- [x] **P1 大数据量 · 第一半：虚拟滚动**：新增 `ICEVirtualList`（窗口计算抽成纯函数
      `computeVirtualRange`，组件只把窗口映射成节点；滚动时换窗口、节点数有上界）。
...
- `ICETable`：单元格自定义编辑器 ✅（`column.editor`）、编辑 Tab 流转 ✅（含自动滚动）、列版式导出/还原 ✅、列头拖拽换序 ✅（含落点指示线）、编辑校验可见提示 ✅、宽表 Tab 横向滚 ✅、回车流转向下 ✅、空值策略 ✅
- `ICEUpload`：文件列表 ✅、进度 ✅、`onRemove` ✅、自定义上传 ✅（含重试）、桌面拖放 ✅、上传队列 ✅（并发可调 + 暂停/继续 + 顺序跟列表）
