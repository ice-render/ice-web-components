...
- [工具函数](./helpers.md) — `attachTooltip` `attachPopover` `attachPopconfirm` `attachDropdown` `openModal` `openDrawer` `getICEOverlayManager` `getICEFocusManager` `getICEMessageManager` `getICEWorldBox` `tween` `fadeIn` `fadeOut` `fadeTo` `slideIn` `scaleIn` `estimateTextWidth` `formatStatisticValue` `formatCountdown` `truncateTextLines` `tooltipPanelWidth` `readHovered` `createTextNode` `centerTextNode` `getStatusColors` `resolveICEEasing` `easeInQuad` `easeOutCubic` `easeInOutCubic` `resolveICEOverlayPosition` `isPointInsideICEBox` `iceUIManager` `ICE_LIGHT_THEME` `ICE_DARK_THEME`
