...
表单容器：把若干 ICEFormItem 纵向堆叠，绑上校验模型。  纵向堆叠 + 每个表单项拉满宽度交给**引擎的箱式布局**（`ICEBoxLayout({ axis: 'y', align: 'stretch' })`， `stretch` 就是 Swing BoxLayout 的默认口径）；本组件只保留一条自己的策略： **高度等于内容高度**（`doLayout()` 之后同步一次）。  ⚠️ `stretch` 拉的是**表单项**（`ICEFormItem`），**不是表单里的控件** —— 控件宽度是它自己的属性， 所以只写 `stretch` 会让表单右边空掉一截（不报错）。完整口径见 `docs/guides/layout.md`。
...
**构造参数** `ICEFormOptions` — 表单容器：把若干 ICEFormItem 纵向堆叠，绑上校验模型。  纵向堆叠 + 每个表单项拉满宽度交给**引擎的箱式布局**（`ICEBoxLayout({ axis: 'y', align: 'stretch' })`， `stretch` 就是 Swing BoxLayout 的默认口径）；本组件只保留一条自己的策略： **高度等于内容高度**（`doLayout()` 之后同步一次）。  ⚠️ `stretch` 拉的是**表单项**（`ICEFormItem`），**不是表单里的控件** —— 控件宽度是它自己的属性， 所以只写 `stretch` 会让表单右边空掉一截（不报错）。完整口径见 `docs/guides/layout.md`。
