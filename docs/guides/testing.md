...
| 脚本 | 页面 | 项数 | 覆盖 |
|---|---|---|---|
| `qa:admin` | `examples/admin.html` | 52 | 6 个页面顶层零交叠 / 首元素边距一致；逐页新组件（面包屑、浮动按钮+引导、回到顶部、库存分页、履约分栏拖动与锚点、图片预览、订单多选与批量发货、跨字段校验、通知渠道上限）；全部弹层开关 + Esc；**文本输入：原生替身挂载/中文 insertText/IME 组字/失焦收起**；**交互后几何：提示盒=文字实测宽度 / 提示与按钮不重叠且按钮左缘可点 / 清空选择换长文案后仍不重叠 / 搜索框左下角可聚焦**；零 console error |
