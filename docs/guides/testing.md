...
| `qa:arcade` | `examples/arcade.html` | 45 | 俄罗斯方块卡带：真实按键驱动（← → 移动、↑ 旋转、↓ 软降、空格硬降、P 暂停且重力真的停、R 重开）、造题强制消行、一路硬降到 game over 并写入最高分；**换卡带到贪吃蛇**：棋盘/HUD 卡片换掉、真实方向键入队、喂食长身子、撞墙结束并记最高分、暂停时 tick 无效、R 重开、切回第一块卡带是新模型；鼠标点 HUD 按钮与音效开关；两种卡带的棋盘都在屏幕框内、面板不交叠 |
