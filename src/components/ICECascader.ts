...
export class ICECascader extends ICEComponent {
  ...
  private placement: ICECascaderPlacement;
  private manager: ICEOverlayManager | null;
  private onChangeCallback: ((value: string, path: ICECascaderOption[]) => void) | null;
  ...
  /** 已选路径（根 → 叶子）；未选时为空数组。 */
  public getPath(): ICECascaderOption[] {
  ...
  public getColumnNodes(): ICEScrollPane[] {
    return this.columns.slice();
  }

  public getColumnNode(level: number): ICEScrollPane | null {
  ...
  public open(): this {
    ...
    if (!this.manager) {
      if (!this.ice) {
        throw new Error('ICECascader 需要先加入 ICE 场景');
      }
      this.manager = getICEOverlayManager(this.ice);
    }
    this.activePath = this.__findPath(this.value);
  ...
  private __findPath(value: string | undefined): ICECascaderOption[] {
    if (value === undefined) {
      return [];
    }
    const walk = (nodes: ICECascaderOption[], trail: ICECascaderOption[]): ICECascaderOption[] | null => {
  ...
  private __maxDepth(): number {
    const walk = (nodes: ICECascaderOption[]): number => {
      let depth = 1;
      for (const node of nodes) {
        if (node.children && node.children.length) {
  ...
  private __pick(node: ICECascaderOption, level: number): void {
    if (node.disabled) {
      return;
    }
  ...
  private __renderPanel(): void {
    ...
    const levels: ICECascaderOption[][] = [this.options];
