...
export class UIAutoComplete extends UIComponent {
  private allOptions: string[];
  private field: UITextField;
  ...
  constructor(props: UIAutoCompleteOptions) {
  ...
  private __overlayManager(): UIOverlayManager {
    if (!this.manager) {
      if (!this.ice) {
        throw new Error('UIAutoComplete 需要先加入 ICE 场景');
      }
      this.manager = getUIOverlayManager(this.ice);
    }
    return this.manager;
