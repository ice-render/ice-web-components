...
export type UIButtonVariant = 'primary' | 'default' | 'text' | 'link';
export type UIButtonSize = 'small' | 'middle' | 'large';

function heightForSize(theme: any, size: UIButtonSize): number {
  if (size === 'small') return theme.control.heightSmall;
  if (size === 'large') return theme.control.heightLarge;
  return theme.control.height;
}

function fontSizeForSize(theme: any, size: UIButtonSize): number {
...
function styleFor(
  ...
  return {
    ...
    fillStyle: fill,
    strokeStyle: fill,
    shadow: enabled ? theme.shadows.sm : undefined,
  };
}

function textColor(theme: any, variant: UIButtonVariant, danger: boolean, enabled: boolean, hovered = false): string {
