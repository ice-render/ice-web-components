 ...
 */
export interface UIColorPickerOptions {
  colors?: string[];
  columns?: number;
  value?: string;
  swatchSize?: number;
...
export class UIColorPicker extends UIComponent {
  ...
  private cells: UIComponent[] = [];
  private running = false;

  constructor(props: UIColorPickerOptions = {}) {
    const theme = uiManager.getTheme();
    const colors = (props.colors && props.colors.length ? props.colors : UIColorPicker.__defaultPalette()).slice();
    const columns = Math.max(1, Math.floor(props.columns ?? 6));
    const gap = props.gap ?? 8;
    const padding = 8;
    const rows = colors.length === 0 ? 0 : Math.ceil(colors.length / columns);
    const size = UIColorPicker.__resolveSwatchSize(props, columns, gap, padding);
  ...
  public getSwatchNodes(): UIComponent[] {
    return this.cells.slice();
  }

  public getSwatchNode(color: string): UIComponent | null {
