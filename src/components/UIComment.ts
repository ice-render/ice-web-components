...
export interface UICommentAction {
  key: string;
  text: string;
}

export interface UICommentOptions {
  author: string;
  content: string;
  time?: string;
  /** 文字头像（默认取 author 首字） */
  avatarText?: string;
  avatarColor?: string;
  actions?: UICommentAction[];
  onAction?: (key: string) => void;
  replies?: UICommentOptions[];
  left?: number;
  top?: number;
  width?: number;
  indent?: number;
...
export class UIComment extends UIComponent {
  private actionButtons = new Map<string, UIButton>();
  private replyNodes: UIComment[] = [];

  constructor(props: UICommentOptions) {
    ...
    if (hasActions) {
      ...
      (props.actions as UICommentAction[]).forEach((action) => {
  ...
  public getReplyNodes(): UIComment[] {
    return this.replyNodes.slice();
  }
}

/** 估算回复高度（避免构造前拿不到子项高度导致父高度偏小）。 */
function estimateReplyHeight(reply: UICommentOptions): number {
