...
export type UIDatePickerPlacement = 'bottomLeft' | 'bottomRight';
...
export interface UIDatePickerOptions {
  ...
  height?: number;
  cellSize?: number;
  placement?: UIDatePickerPlacement;
  onChange?: (value: string) => void;
  manager?: UIOverlayManager;
}

export interface UIDateCell {
  date: string;
  day: number;
  inMonth: boolean;
  isToday: boolean;
...
export class UIDatePicker extends UIComponent {
  ...
  private placement: UIDatePickerPlacement;
  ...
  private monthLabel: UILabel | null = null;
  private grid: UIComponent | null = null;
  private dayNodes = new Map<string, UIComponent>();
  private viewYear = 1970;
  private viewMonth = 1;
  private running = false;

  constructor(props: UIDatePickerOptions = {}) {
  ...
  public getDayNode(date: string): UIComponent | null {
    return this.dayNodes.get(date) || null;
  }

  /** 当前视图月的 6×7 网格（周一开头，含上下月补位）。 */
  public getDayCells(): UIDateCell[] {
  ...
  public open(): this {
    ...
    if (!this.manager) {
      if (!this.ice) {
        throw new Error('UIDatePicker 需要先加入 ICE 场景');
      }
      this.manager = getUIOverlayManager(this.ice);
    }
  ...
  private __renderPanel(): void {
    ...
    // 月份标题 + 切月按钮（用可点的 UIComponent 做，避免额外依赖按钮样式）
    const prev = new UIComponent({
      left: padding,
      top: 6,
      width: 24,
      height: 24,
