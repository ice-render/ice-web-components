...
export interface UIDescriptionsOptions {
  items: UIDescriptionsItem[];
  column?: number;
  left?: number;
  top?: number;
  width?: number;
...
export class UIDescriptions extends UIComponent {
  private items: UIDescriptionsItem[];
  private column: number;
  private itemHeight: number;
  private labelWidth: number;
  private rowNodes: UIComponent[] = [];

  constructor(props: UIDescriptionsOptions) {
    ...
    super({
      ...
      radius: uiManager.getTheme().radius.md,
      style: { fillStyle: uiManager.getTheme().colors.surface, strokeStyle: uiManager.getTheme().colors.border },
    });
  ...
  public getRowNodes(): UIComponent[] {
