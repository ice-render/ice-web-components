 ...
 * 下拉菜单：点击触发组件弹出选项列表。
 *
 * - 弹出/定位/点外关闭/Esc 全部复用 `UIOverlayManager`；
...
export interface UIDropdownOptions {
  ...
  itemHeight?: number;
  offset?: number;
  onSelect?: (item: UIDropdownItem, index: number) => void;
...
export class UIDropdown {
  private ice: any;
  private target: any;
  private options: UIDropdownOptions;
  private manager: UIOverlayManager;
  private handle: UIOverlayHandle | null = null;
  private panel: UIPanel | null = null;
  private itemNodes: UIComponent[] = [];
  private items: UIDropdownItem[];
  private selectedKey: string | null;
  private activeIndex = 0;
  private running = false;

  constructor(ice: any, target: any, options: UIDropdownOptions) {
    this.ice = ice;
    this.target = target;
  ...
  public getItemNode(index: number): UIComponent | null {
    return this.itemNodes[index] || null;
  }

  public setItems(items: UIDropdownItem[]): this {
...
export function attachDropdown(ice: any, target: any, options: UIDropdownOptions): UIDropdown {
  return new UIDropdown(ice, target, options).start();
}
