 ...
 * - 点击目标会 toggle；点浮层外、按 Esc 关闭（由 UIOverlayManager 负责）。
 */
...
export class UIPopover {
  ...
  protected options: UIPopoverOptions;
  protected manager: UIOverlayManager;
  protected handle: UIOverlayHandle | null = null;
  private enterTimer: any = null;
  private leaveTimer: any = null;
  private running = false;

  constructor(ice: any, target: any, options: UIPopoverOptions = {}) {
    this.ice = ice;
    this.target = target;
...
export function attachPopover(ice: any, target: any, options: UIPopoverOptions = {}): UIPopover {
  return new UIPopover(ice, target, options).start();
}
