 ...
 * 结构：
 * ```
 * UIScrollPane (clipChildren: true)
...
export interface UIScrollPaneOptions {
  width?: number;
  height?: number;
  /** 初始滚动位置 */
  scrollX?: number;
...
export class UIScrollPane extends UIContainer {
  private contentBox: UIComponent;
  private contentNode: any = null;
  private scrollbarTrack: UIComponent;
  private scrollbarThumb: UIComponent;
  ...
  constructor(props: UIScrollPaneOptions & Record<string, any> = {}) {
    ...
    this.contentBox = new UIComponent({ left: 0, top: 0, width: 0, height: 0, fill: false, stroke: false });
    this.addChild(this.contentBox, false);

    this.scrollbarTrack = new UIComponent({
    ...
    this.scrollbarThumb = new UIComponent({
      left: 0,
      top: 0,
      width: SCROLLBAR_WIDTH,
      height: 0,
