 ...
 * - 下拉走 `UIOverlayManager`（工具层、点外关闭、Esc、入场动画），并声明 `keyboardCaptured`，
 *   打开期间方向键/Enter/字符输入由本组件处理（搜索过滤）；
 * - 可聚焦（Tab 可达），`activate()`（Enter/Space）打开下拉开；
 * - 表单集成：`getFormValue` / `setFormValue` 与 UITextField 同语义。
 */
...
export class UISelect extends UIComponent {
  ...
  private visible: UISelectOption[] = [];
  private query = '';
  private activeIndex = 0;
  private running = false;

  constructor(props: UISelectOptions) {
  ...
  /** 供测试/外部注入浮层管理器 */
  public setOverlayManager(manager: UIOverlayManager): this {
    this.manager = manager;
    return this;
  }

  private __overlayManager(): UIOverlayManager {
    if (!this.manager) {
      if (!this.ice) {
        throw new Error('UISelect 需要先加入 ICE 场景（或显式 setOverlayManager）');
      }
      this.manager = getUIOverlayManager(this.ice);
    }
    return this.manager;
  ...
  public getVisibleOptions(): UISelectOption[] {
    return this.visible.slice();
  }

  public getOptionNode(value: string): UIComponent | null {
    return this.optionNodes.get(value) || null;
  }

  public setOptions(options: UISelectOption[]): this {
  ...
  private __pick(option: UISelectOption): void {
