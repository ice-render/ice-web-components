...
export interface UIStepsOptions {
  items: UIStepsItem[];
  current?: number;
  left?: number;
  top?: number;
...
export class UISteps extends UIComponent {
  private items: UIStepsItem[];
  private current: number;
  private circleSize: number;
  private stepNodes: UIComponent[] = [];
  private onChange: ((current: number) => void) | null;

  constructor(props: UIStepsOptions) {
  ...
  public getStepNode(index: number): UIComponent | null {
