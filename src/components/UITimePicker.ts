 ...
 * - 浮层是「时/分/秒」三列（`format: 'HH:mm'` 时只有两列），列内用 `UIScrollPane` 滚动，
 *   打开时自动滚到当前取值；
 * - 取值受步进控制（`hourStep` / `minuteStep` / `secondStep`）；
 * - 点某个取值 → 只改该单位 → 回写值 + 关闭 + `onChange`（与 `UIDatePicker` 的交互一致）；
 * - 关闭策略与 Select 系列一致：浮层 closeOnOutsideClick:false，由组件自己判断点外 + Esc。
 */

export type UITimeUnit = 'hour' | 'minute' | 'second';
export type UITimePickerFormat = 'HH:mm:ss' | 'HH:mm';
export type UITimePickerPlacement = 'bottomLeft' | 'bottomRight';

export interface UITimePickerOptions {
  value?: string;
  placeholder?: string;
  disabled?: boolean;
  /** 值格式：HH:mm:ss（默认）或 HH:mm */
  format?: UITimePickerFormat;
  hourStep?: number;
  minuteStep?: number;
  secondStep?: number;
  left?: number;
  ...
  width?: number;
  height?: number;
  placement?: UITimePickerPlacement;
...
export class UITimePicker extends UIComponent {
  private value?: string;
  private placeholder: string;
  private disabled: boolean;
  private format: UITimePickerFormat;
  private steps: Record<UITimeUnit, number>;
  private placement: UITimePickerPlacement;
  ...
  private columnNodes = new Map<UITimeUnit, UIScrollPane>();
  private optionNodes = new Map<string, UIComponent>();
  private running = false;

  constructor(props: UITimePickerOptions = {}) {
  ...
  public getColumnNode(unit: UITimeUnit): UIScrollPane | null {
    return this.columnNodes.get(unit) || null;
  }

  public getOptionNode(unit: UITimeUnit, value: string): UIComponent | null {
  ...
  public open(): this {
    ...
    if (!this.manager) {
      if (!this.ice) {
        throw new Error('UITimePicker 需要先加入 ICE 场景');
      }
      this.manager = getUIOverlayManager(this.ice);
    }
    const theme = uiManager.getTheme();
  ...
  private __pick(unit: UITimeUnit, value: string): void {
  ...
  private __renderPanel(): void {
    ...
    this.optionNodes.clear();
    const parts = this.__parse();
    const current: Record<UITimeUnit, string> = {
      hour: pad(parts.hour),
