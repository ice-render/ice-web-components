...
export interface UITransferItem {
  key: string;
  title: string;
  description?: string;
  disabled?: boolean;
...
export type UITransferDirection = 'left' | 'right';

export interface UITransferOptions {
  dataSource: UITransferItem[];
  targetKeys?: string[];
  ...
  height?: number;
  rowHeight?: number;
  onChange?: (targetKeys: string[], direction: UITransferDirection, moveKeys: string[]) => void;
}
...
export class UITransfer extends UIComponent {
  private dataSource: UITransferItem[];
  private targetKeys: string[];
  private titles: [string, string];
  private rowHeight: number;
  private onChangeCallback:
    | ((targetKeys: string[], direction: UITransferDirection, moveKeys: string[]) => void)
    | null;
  private checked = new Set<string>();
  private sourceRows = new Map<string, UIComponent>();
  private targetRows = new Map<string, UIComponent>();
  private moveRightButton: UIComponent | null = null;
  private moveLeftButton: UIComponent | null = null;

  constructor(props: UITransferOptions) {
  ...
  public getSourceNode(key: string): UIComponent | null {
    return this.sourceRows.get(key) || null;
  }

  public getTargetNode(key: string): UIComponent | null {
    return this.targetRows.get(key) || null;
  }

  public getMoveRightButton(): UIComponent | null {
    return this.moveRightButton;
  }

  public getMoveLeftButton(): UIComponent | null {
  ...
  private __emit(direction: UITransferDirection, moveKeys: string[]): void {
  ...
  private __movable(keys: string[], direction: UITransferDirection): string[] {
    const known = new Map(this.dataSource.map((item) => [item.key, item]));
  ...
  private __renderRow(item: UITransferItem, top: number, width: number): UIComponent {
    const theme = uiManager.getTheme();
  ...
  private __renderMoveButton(text: string, left: number, top: number, enabled: boolean): UIComponent {
    const theme = uiManager.getTheme();
