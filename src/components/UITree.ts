 ...
 * - 选择走 `UISelectionModel`（single / multiple）；
 * - 键盘：↑/↓ 移动激活行、→ 展开（已展开则进入首个子节点）、← 折叠（叶子则回父节点）、
 *   Enter/Space 选中；
 * - 内容超出可视高度时自动套 `UIScrollPane`。
 */
...
export interface UITreeOptions {
  ...
  itemHeight?: number;
  indent?: number;
  onSelect?: (keys: string[], node?: UITreeNode) => void;
...
export class UITree extends UIComponent {
  ...
  private onSelect: ((keys: string[], node?: UITreeNode) => void) | null;
  ...
  constructor(props: UITreeOptions) {
    ...
    this.content = new UIComponent({ left: 0, top: 0, width: width - 8, height });
    this.focusable = true;
    this.__render();
  }
  ...
  public getVisibleNodes(): UITreeNode[] {
  ...
  private __flatten(): FlatRow[] {
    const rows: FlatRow[] = [];
    const visit = (list: UITreeNode[], depth: number, parentKey: string | null) => {
      list.forEach((node) => {
        const hasChildren = !!(node.children && node.children.length);
        rows.push({ node, depth, parentKey, hasChildren });
        if (hasChildren && this.expanded.indexOf(node.key) !== -1) {
          visit(node.children as UITreeNode[], depth + 1, node.key);
        }
      });
    };
  ...
  private __collectExpandableKeys(list: UITreeNode[]): string[] {
    const keys: string[] = [];
    const visit = (nodes: UITreeNode[]) => {
      nodes.forEach((node) => {
  ...
  private __syncRows(): void {
    ...
    this.rows.forEach((row, index) => {
      ...
      // 点击缩进区域（箭头附近）也算展开
      const arrowHit = new UIComponent({
        left: indent - 2,
        top: 0,
