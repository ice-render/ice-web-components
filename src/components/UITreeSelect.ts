...
import { UITree, UITreeNode } from './UITree';
import { uiManager } from '../core/UIManager';
import { UIOverlayManager, UIOverlayHandle, getUIOverlayManager } from '../core/UIOverlayManager';

/**
 * 树选择器（业界组件库 TreeSelect）：下拉里放一棵 UITree，选中节点后回写值。
 *
 * - 字段区与 UISelect 同构（选中标签 / placeholder + ▾），错误态边框标红；
 * - 下拉内容直接复用 `UITree`（层级展开、缩进、箭头命中区、选择模型都在那边）；
 * - 浮层自己管关闭（`closeOnOutsideClick: false` + 自身命中盒）—— 浮层的命中盒判定会在
 *   click 派发前关掉浮层，树节点就收不到点击（Select/AutoComplete 都踩过这个坑）；
 * - 值就是节点 key，表单语义同 UITextField（getFormValue / setFormValue）。
 */

export interface UITreeSelectOptions {
  nodes: UITreeNode[];
  value?: string;
  placeholder?: string;
  disabled?: boolean;
  ...
  onChange?: (key: string, node: UITreeNode) => void;
  manager?: UIOverlayManager;
}

export class UITreeSelect extends UIComponent {
  private nodes: UITreeNode[];
  ...
  private tree: UITree | null = null;
  private fieldLabel: UILabel | null = null;
  private onChangeCallback: ((key: string, node: UITreeNode) => void) | null;
  private running = false;

  constructor(props: UITreeSelectOptions) {
  ...
  public open(): this {
    ...
    if (!this.manager) {
      if (!this.ice) {
        throw new Error('UITreeSelect 需要先加入 ICE 场景');
      }
      this.manager = getUIOverlayManager(this.ice);
    }
    const theme = uiManager.getTheme();
    ...
    const tree = new UITree({
      ...
      onSelect: (keys: string[], node?: UITreeNode) => {
        if (node) {
          this.__pick(node);
        }
      },
  ...
  private __pick(node: UITreeNode): void {
    this.value = node.key;
    this.__syncField();
    this.close();
  ...
  private __findNode(key: string): UITreeNode | null {
    const visit = (list: UITreeNode[]): UITreeNode | null => {
