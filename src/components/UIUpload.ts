...
export interface UIUploadOptions {
  accept?: string;
  multiple?: boolean;
  maxCount?: number;
  /** 单文件大小上限（字节） */
  ...
  height?: number;
  rowHeight?: number;
  beforeUpload?: (file: UIUploadFile) => boolean | string | undefined;
  onChange?: (files: UIUploadFile[]) => void;
}

const DROP_ZONE_HEIGHT = 96;

export class UIUpload extends UIComponent {
  private files: UIUploadFile[] = [];
  ...
  private beforeUpload: ((file: UIUploadFile) => boolean | string | undefined) | null;
  private onChangeCallback: ((files: UIUploadFile[]) => void) | null;
  private lastRejectReason: string | null = null;
  private dropZone: UIComponent | null = null;
  private fileNodes = new Map<string, UIComponent>();
  private inputEl: any = null;
  private seq = 0;

  constructor(props: UIUploadOptions = {}) {
  ...
  public getFileList(): UIUploadFile[] {
  ...
  public getDropZoneNode(): UIComponent | null {
    return this.dropZone;
  }

  public getFileNode(uid: string): UIComponent | null {
    return this.fileNodes.get(uid) || null;
  }

  /** 加入一个文件（真实选择结果或调用方构造的数据）；被拒时返回 false。 */
  public addFile(file: Partial<UIUploadFile>): boolean {
    const normalized: UIUploadFile = {
  ...
  private __rejectReason(file: UIUploadFile): string | null {
    if (this.disabled) {
      return '已禁用';
    }
  ...
  private __accepts(file: UIUploadFile): boolean {
    if (!this.accept) {
      return true;
    }
