...
export type ICEOverlayCloseReason = 'api' | 'outside' | 'esc' | 'exclusive';
...
interface ICEOverlayEntry {
  handle: ICEOverlayHandle;
  anchor: any;
  content: any;
  options: ICEOverlayOptions;
}
...
export class ICEOverlayManager {
  private ice: any;
  private layer: any = null;
  private entries: ICEOverlayEntry[] = [];
  private bound = false;
  ...
  public open(options: ICEOverlayOptions): ICEOverlayHandle {
    const content = options && options.content;
    if (!content) {
      throw new Error('ICEOverlayManager.open() 需要 content');
    }
    this.start();
    ...
    const entry: ICEOverlayEntry = { handle: null as any, anchor: options.anchor, content, options };
    const handle: ICEOverlayHandle = {
  ...
  public close(handle: ICEOverlayHandle, reason: ICEOverlayCloseReason = 'api'): void {
  ...
  public closeAll(reason: ICEOverlayCloseReason = 'api'): void {
  ...
  private __destroyEntry(entry: ICEOverlayEntry, reason: ICEOverlayCloseReason): void {
    const { layer } = this;
  ...
  private __place(entry: ICEOverlayEntry): void {
    ...
    const position = resolveICEOverlayPosition({
...
/** 每个 ICE 实例一个管理器（与 ICEHoverManager 的「显式启动」风格一致，但可懒创建）。 */
const managers = new WeakMap<object, ICEOverlayManager>();

export function getICEOverlayManager(ice: any): ICEOverlayManager {
  if (!ice || typeof ice !== 'object') {
    throw new Error('getICEOverlayManager(ice) 需要一个 ICE 实例');
  }
