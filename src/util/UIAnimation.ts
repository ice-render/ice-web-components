...
export type UIEasingName = 'linear' | 'easeIn' | 'easeOut' | 'easeInOut';
export type UIEasing = UIEasingName | ((t: number) => number);

export interface UIFrameDriver {
  request(callback: (time: number) => void): any;
  cancel(handle: any): void;
}

export interface UITweenOptions {
  from?: number;
  to: number;
  /** 毫秒；≤ 0 或非法时同步落到终点 */
  duration?: number;
  ...
  onFinish?: () => void;
  driver?: UIFrameDriver;
}

export interface UITweenHandle {
  cancel(): void;
}

const defaultDriver: UIFrameDriver = {
  request(callback) {
...
export function resolveUIEasing(easing?: UIEasing): (t: number) => number {
...
/** 过渡公共参数（不含 from/to）。 */
export interface UITransitionOptions {
  duration?: number;
  delay?: number;
  easing?: UIEasing;
  onFinish?: () => void;
...
/** 把不透明度过渡到指定值。 */
export function fadeTo(component: any, to: number, options: UITransitionOptions = {}): UITweenHandle {
...
/** 淡入：从 0 到 1。 */
export function fadeIn(component: any, options: UITransitionOptions = {}): UITweenHandle {
...
export function fadeOut(component: any, options: UITransitionOptions = {}): UITweenHandle {
  return fadeTo(component, 0, { duration: 150, ...options });
}

export interface UISlideOptions extends UITransitionOptions {
  /** 从哪一侧滑入（目标位置保持不变） */
...
export function slideIn(component: any, options: UISlideOptions): UITweenHandle {
  ...
  let opacityHandle: UITweenHandle | null = null;
  if (options.fade !== false) {
    opacityHandle = tween({
      from: 0,
      to: 1,
...
export interface UIScaleInOptions extends UITransitionOptions {
  /** 起始缩放（默认 0.96） */
  from?: number;
  /** 是否同时淡入，默认 true */
  fade?: boolean;
...
export function scaleIn(component: any, options: UIScaleInOptions = {}): UITweenHandle {
