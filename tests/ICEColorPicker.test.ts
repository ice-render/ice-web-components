...
describe('ICEColorPicker', () => {
  it('渲染色块网格与选中环', () => {
    const picker = new ICEColorPicker({ width: 200, colors: palette, columns: 6, value: '#52c41a' });
  ...
  it('点击色块回写值并回调；disabled 忽略', () => {
    ...
    const picker = new ICEColorPicker({ width: 200, colors: palette, columns: 6, onChange: (color) => changes.push(color) });
    ...
    const disabled = new ICEColorPicker({ width: 200, colors: palette, columns: 6, disabled: true });
  ...
  it('setValue 同步选中；表单取值约定与错误态', () => {
    const picker = new ICEColorPicker({ width: 200, colors: palette, columns: 6 });
  ...
  it('rows 计算：6 列 6 色 → 1 行；高度按行数', () => {
    const picker = new ICEColorPicker({ width: 200, colors: palette, columns: 3, swatchSize: 24, gap: 8 });
