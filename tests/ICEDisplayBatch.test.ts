/**
 * 展示类组件批次单测：ICESegmented / ICEEmpty / ICESkeleton。
 */
...
describe('ICESegmented', () => {
  ...
  it('渲染所有选项；点击切换选中并回调', () => {
    ...
    const segmented = new ICESegmented({ width: 240, options, value: 'day', onChange: (v) => changes.push(v) });
  ...
  it('setValue 更新选中态（选中项为 primary 实心）', () => {
    const segmented = new ICESegmented({ width: 240, options, value: 'day' });
...
describe('ICESkeleton', () => {
  ...
  it('setActive 切换呼吸动画状态（不改变占位结构）', () => {
    const skeleton = new ICESkeleton({ width: 240, rows: 2 });
