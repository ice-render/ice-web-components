...
function makeScene() {
  const ice = makeICE();
  const panel = new ICEPanel({ left: 0, top: 0, width: 400, height: 200 });
  const button = new ICEButton({ left: 10, top: 10, width: 100, height: 32, text: 'OK' });
  const checkbox = new ICECheckBox({ left: 10, top: 60, width: 24, height: 24 });
  const switchCtl = new ICESwitch({ left: 10, top: 100, width: 44, height: 22 });
  panel.addChildren([button, checkbox, switchCtl]);
  ice.childNodes = [panel];
  return { ice, panel, button, checkbox, switch: switchCtl };
}

describe('ICEFocusManager', () => {
  ...
  it('焦点范围（模态焦点陷阱）：Tab 只在范围内轮转，范围外的焦点被清掉', () => {
    ...
    const dialog = new ICEPanel({ left: 20, top: 40, width: 200, height: 120 });
    const dialogButton = new ICEButton({ left: 10, top: 10, width: 80, height: 28, text: 'Confirm' });
    const dialogSwitch = new ICESwitch({ left: 10, top: 60, width: 44, height: 22 });
