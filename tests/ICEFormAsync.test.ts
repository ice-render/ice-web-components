 ...
 * - `ICEForm.submitAsync()` 等异步校验通过才回调 onSubmit。
 */
...
describe('ICEFormModel 异步校验', () => {
  ...
  it('值变化只跑同步规则（异步校验不自动发请求）', () => {
    let called = 0;
    const model = new ICEFormModel({ validateTrigger: 'change' });
    model.addField({
      name: 'nick',
      label: '昵称',
      value: 'old',
...
describe('ICEForm 异步提交', () => {
  it('异步校验不过不回调 onSubmit；通过才回调', async () => {
    ...
    const item = new ICEFormItem({
      name: 'nick',
      label: '昵称',
      control: new ICETextField({ value: 'x', width: 200 }),
    });
    form.addItem(item);
    const gate = deferred();
