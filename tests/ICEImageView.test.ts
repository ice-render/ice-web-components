...
describe('ICEImageView', () => {
  it('computeFit：fill / contain / cover 三种适配', () => {
    const fill = ICEImageView.computeFit(200, 100, 100, 100, 'fill');
    expect(fill).toEqual({ left: 0, top: 0, width: 100, height: 100 });

    const contain = ICEImageView.computeFit(200, 100, 100, 100, 'contain');
    expect(contain).toEqual({ left: 0, top: 25, width: 100, height: 50 });

    const cover = ICEImageView.computeFit(200, 100, 100, 100, 'cover');
    expect(cover).toEqual({ left: -50, top: 0, width: 200, height: 100 });
  });

  it('construct 出图片原语，src 可读写，默认未加载', () => {
    const image = new ICEImageView({ src: 'a.png', width: 120, height: 80 });
  ...
  it('未知原始尺寸时占满容器盒；setNaturalSize 后按 fit 重排', () => {
    const image = new ICEImageView({ src: 'a.png', width: 100, height: 100, fit: 'contain' });
