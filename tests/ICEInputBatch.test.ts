/**
 * 录入类批次：ICEInputNumber / ICEAutoComplete。
 *
 * ICEInputNumber 规格：
 ...
 * - 表单集成与错误态同 ICETextField。
 */
import { ICEAutoComplete } from '../src/components/ICEAutoComplete';
import { ICEInputNumber } from '../src/components/ICEInputNumber';

describe('ICEInputNumber', () => {
  it('步进加减 + 夹取 + precision', () => {
    const changes: number[] = [];
    const input = new ICEInputNumber({ width: 140, value: 5, min: 0, max: 10, step: 2, onChange: (v) => changes.push(v) });
  ...
  it('precision：步进结果按精度取整', () => {
    const input = new ICEInputNumber({ width: 140, value: 1, step: 0.1, precision: 1 });
    input.getIncreaseButton()!.trigger('click', null, {});
    expect(input.getValue()).toBe(1.1);
  });

  it('键盘 ↑/↓ 步进，直接输入数字与小数点', () => {
    const input = new ICEInputNumber({ width: 140, value: 1, step: 1, max: 100 });
  ...
  it('disabled 不响应；表单取值约定', () => {
    const input = new ICEInputNumber({ width: 140, value: 5, disabled: true });
    input.getIncreaseButton()!.trigger('click', null, {});
    expect(input.getValue()).toBe(5);
...
function setup() {
  ...
  const autocomplete = new ICEAutoComplete({
    left: 0,
    top: 0,
    width: 200,
    options: ['杭州', '上海', '深圳', '北京'],
