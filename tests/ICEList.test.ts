 ...
 *
 * 规格：
 * - 行由 items 渲染，选中态高亮；单选替换、多选切换（走 ICESelectionModel）；
 * - 点击行选中；disabled 行忽略；
 * - 键盘（焦点在列表上时）：↑/↓ 移动激活行（跳过 disabled），Enter/Space 选中当前激活行；
 * - 内容超出高度时套 ICEScrollPane 承载（可滚动），否则不需要滚动条；
 * - onChange(keys) 回调。
 */
...
describe('ICEList', () => {
  ...
  it('内容超出高度时用 ICEScrollPane 承载，否则不用', () => {
    const short = new ICEList({ width: 200, height: 200, items });
    expect(short.getScrollPane()).toBeNull();

    const tall = new ICEList({ width: 200, height: 80, items, itemHeight: 36 });
    const pane = tall.getScrollPane() as ICEScrollPane | null;
  ...
  it('setItems / setSelectedKeys / getValue 语义', () => {
    const list = new ICEList({ width: 200, height: 160, mode: 'multiple', items });
