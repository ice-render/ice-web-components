 ...
 * - 页码按钮是 ICEButton（可聚焦，键盘 Enter 激活）。
 */
...
function buttonsOf(pagination: ICEPagination): any[] {
  return (pagination.childNodes || []).filter((node: any) => typeof node.activate === 'function' && node.state.text !== undefined);
}

describe('ICEPagination', () => {
  it('页数计算与 current 夹取', () => {
    const pagination = new ICEPagination({ width: 420, total: 95, pageSize: 10 });
  ...
  it('渲染：首页时上一页不可用、当前页高亮、含页码与共 N 条', () => {
    const pagination = new ICEPagination({ width: 420, total: 95, pageSize: 10, showTotal: true });
  ...
  it('页数很多时用省略号收口（最多 7 个页码位）', () => {
    const pagination = new ICEPagination({ width: 420, total: 500, pageSize: 10 });
