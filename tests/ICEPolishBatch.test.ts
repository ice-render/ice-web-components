...
describe('ICEAlert 可关闭', () => {
  ...
  it('点关闭按钮：隐藏自己并回调 onClose（只回调一次）', () => {
    let closed = 0;
    const alert = new ICEAlert({ title: 't', message: 'm', closable: true, onClose: () => (closed += 1) });
  ...
  it('非 closable 也能用 close() 编程式关闭', () => {
    const alert = new ICEAlert({ title: 't' });
    alert.close();
    expect(alert.isClosed()).toBe(true);
  });
});

describe('ICEAvatarGroup 头像组', () => {
  const avatars = [
    { text: 'A' },
    { text: 'B' },
    { text: 'C' },
  ...
  it('超出 max 时折叠成 +N，并算出可见数量', () => {
    const group = new ICEAvatarGroup({ avatars, size: 32, max: 3 });
    ...
    const all = new ICEAvatarGroup({ avatars: avatars.slice(0, 2), size: 32, max: 3 });
  ...
  it('重叠排布：相邻头像左移 overlap', () => {
    const group = new ICEAvatarGroup({ avatars, size: 32, max: 4, overlap: 8 });
