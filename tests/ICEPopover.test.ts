/**
 * ICEPopover / ICEPopconfirm 单测。
 *
 * 规格：
 * - popover 默认 click 触发：点目标 toggle；点浮层外由 ICEOverlayManager 关闭；
 * - hover 触发走进入/离开延时；
...
import { ICEPopconfirm } from '../src/components/ICEPopconfirm';
import { ICEPopover } from '../src/components/ICEPopover';
...
function collectText(node: any): string[] {
  ...
  const walk = (current: any) => {
    // ICELabel 与它内部的 ICEText 都带同一段 text，这里去重
...
function lastLayerContent(manager: ICEOverlayManager): any {
...
describe('ICEPopover', () => {
  it('click 触发：点目标打开、再点关闭（toggle），内容含标题与正文', () => {
    ...
    const target = new ICEButton({ left: 10, top: 10, width: 96, height: 32, text: '更多' });
    const popover = new ICEPopover(ice, target, {
      title: '标题',
      content: '正文内容',
      manager,
      enterAnimation: 'none',
  ...
  it('点浮层与目标之外关闭（交给 ICEOverlayManager）', () => {
    const ice = makeICE();
    const manager = new ICEOverlayManager(ice);
    const target = new ICEButton({ left: 10, top: 10, width: 96, height: 32, text: '更多' });
    const popover = new ICEPopover(ice, target, { content: 'x', manager, exitAnimation: 'none' }).start();
  ...
  it('hover 触发：进入延时后打开、离开延时后关闭', () => {
    ...
    const ice = makeICE();
    const manager = new ICEOverlayManager(ice);
    const target = new ICEButton({ left: 0, top: 0, width: 80, height: 32, text: 't' });
    const popover = new ICEPopover(ice, target, {
      content: '悬停内容',
      trigger: 'hover',
      manager,
  ...
  it('内容工厂每次弹出新建；onOpenChange 报开关；destroy 后不再响应', () => {
    ...
    const popover = new ICEPopover(ice, target, {
      content: () => {
        created += 1;
        const { ICEPanel } = require('../src/components/ICEPanel');
        return new ICEPanel({ width: 40, height: 20 });
      },
      manager,
      enterAnimation: 'none',
      exitAnimation: 'none',
...
describe('ICEPopconfirm', () => {
  function setup(options: any = {}) {
    const ice = makeICE();
    const manager = new ICEOverlayManager(ice);
    const target = new ICEButton({ left: 0, top: 0, width: 96, height: 32, text: '删除' });
    const confirm = new ICEPopconfirm(ice, target, {
      title: '确认删除？',
      description: '删除后不可恢复',
      manager,
      enterAnimation: 'none',
