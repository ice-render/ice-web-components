 ...
 */
import { ICEProgressBar } from '../src/components/ICEProgressBar';

describe('ICEProgressBar 线形（回归）', () => {
  it('保持 track + fill 结构与填充宽度', () => {
    const bar = new ICEProgressBar({ value: 40, min: 0, max: 100, width: 200, height: 10 });
...
describe('ICEProgressBar 环形', () => {
  it('渲染底环 + 进度弧 + 百分比文字，尺寸取 size', () => {
    const bar = new ICEProgressBar({ type: 'circle', value: 25, size: 120 });
  ...
  it('setValue 更新扫过比例与文字', () => {
    const bar = new ICEProgressBar({ type: 'circle', value: 25, size: 100 });
    bar.setValue(50);
  ...
  it('showText:false 不出文字；format 可自定义', () => {
    const plain = new ICEProgressBar({ type: 'circle', value: 30, size: 80, showText: false });
    expect(plain.getTextNode()).toBeNull();

    const custom = new ICEProgressBar({
      type: 'circle',
      value: 30,
      size: 80,
  ...
  it('成功态用成功色，size / strokeWidth 可配', () => {
    const bar = new ICEProgressBar({ type: 'circle', value: 40, size: 90, strokeWidth: 12, status: 'success' });
