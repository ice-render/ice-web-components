 ...
 *
 * 规格：
 * - 悬停进入延时后弹出（内容贴在目标旁边，走 ICEOverlayManager 的定位）；
...
import { ICETooltip } from '../src/components/ICETooltip';
...
function freshManager(ice: any): ICEOverlayManager {
  return new ICEOverlayManager(ice);
}
...
describe('ICETooltip', () => {
  ...
  it('悬停延时后弹出，位置贴住目标；离开延时后关闭', () => {
    ...
    const target = new ICEButton({ left: 100, top: 100, width: 96, height: 32, text: 'Hover me' });
    const tooltip = new ICETooltip(ice, target, {
      title: '提示内容',
      manager,
      mouseEnterDelay: 100,
      mouseLeaveDelay: 80,
  ...
  it('进入延时未到就离开 → 不弹出', () => {
    ...
    const tooltip = new ICETooltip(ice, target, { title: 'x', manager, mouseEnterDelay: 200 }).start();
  ...
  it('custom content：每次弹出新建组件（浮层关闭会销毁内容）', () => {
    ...
    const tooltip = new ICETooltip(ice, target, {
      content: () => {
        ...
        return new ICEPanel({ width: 60, height: 24 });
      },
      manager,
      mouseEnterDelay: 0,
      mouseLeaveDelay: 0,
  ...
  it('destroy() 后不再响应 hover', () => {
    ...
    const manager = freshManager(ice);
    const target = new ICEButton({ left: 0, top: 0, width: 80, height: 32, text: 't' });
    const tooltip = new ICETooltip(ice, target, { title: 'x', manager, mouseEnterDelay: 0 }).start();
    tooltip.destroy();
    target.setHovered(true);
