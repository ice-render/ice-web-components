 ...
 */
import { UICarousel, UICarouselScheduler, UICarouselFrameDriver } from '../src/components/UICarousel';
import { UIComponent } from '../src/core/UIComponent';

function makeSlides(count: number): UIComponent[] {
  return Array.from({ length: count }, (_, i) => new UIComponent({ width: 200, height: 100, id: `slide-${i}` }));
}

function makeScheduler() {
  const tasks = new Map<any, () => void>();
  let handle = 0;
  const scheduler: UICarouselScheduler = {
...
function makeDriver() {
  ...
  let time = 0;
  const driver: UICarouselFrameDriver = {
    request(callback) {
...
describe('UICarousel', () => {
  ...
  it('next / prev 循环；loop:false 到边界停住', () => {
    ...
    const strict = new UICarousel({ width: 200, height: 100, slides: makeSlides(3), duration: 0, loop: false });
    strict.prev();
  ...
  it('箭头与圆点可切换；圆点数量 = 幻灯片数量', () => {
    const carousel = new UICarousel({ width: 200, height: 100, slides: makeSlides(3), duration: 0 });
  ...
  it('setSlides 重建；空列表不崩', () => {
    const carousel = new UICarousel({ width: 200, height: 100, slides: makeSlides(2), duration: 0 });
