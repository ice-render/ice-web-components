/**
 * 展示类组件批次二：UICollapse / UIDescriptions / UITimeline。
 */
import { UICollapse } from '../src/components/UICollapse';
import { UIDescriptions } from '../src/components/UIDescriptions';
import { UITimeline } from '../src/components/UITimeline';
...
describe('UICollapse', () => {
  it('默认全部折叠：只有标题，内容不渲染', () => {
    const collapse = new UICollapse({ width: 280, items: collapseItems });
  ...
  it('accordion：同时只展开一个', () => {
    const collapse = new UICollapse({ width: 280, items: collapseItems, accordion: true, activeKeys: ['a'] });
    expect(collapse.getActiveKeys()).toEqual(['a']);
...
describe('UIDescriptions', () => {
  ...
  it('column=1：逐行渲染 label/value', () => {
    const descriptions = new UIDescriptions({ width: 300, items });
  ...
  it('column=2：两列排布（第二列在右侧）', () => {
    const descriptions = new UIDescriptions({ width: 400, column: 2, items });
...
describe('UITimeline', () => {
  ...
  it('渲染标题/描述/时间，且按顺序自上而下排布', () => {
    const timeline = new UITimeline({ width: 320, items });
  ...
  it('自定义节点颜色与高度', () => {
    const timeline = new UITimeline({ width: 320, items: [{ title: 'A', color: '#ff4d4f' }], itemHeight: 48 });
