/**
 * 展示/反馈批次三：UISpin / UIRate / UISteps / UIResult。
 */
import { UIResult } from '../src/components/UIResult';
import { UIRate } from '../src/components/UIRate';
import { UISpin } from '../src/components/UISpin';
import { UISteps } from '../src/components/UISteps';
...
describe('UISpin', () => {
  it('渲染旋转弧线，可选提示文字', () => {
    const spin = new UISpin({ size: 32, tip: '加载中…' });
  ...
  it('spinning=false 时不注册旋转动画', () => {
    const paused = new UISpin({ size: 24, spinning: false });
    expect(paused.isSpinning()).toBe(false);
    const running = new UISpin({ size: 24 });
...
describe('UIRate', () => {
  it('渲染 N 颗星、点击设置分值并回调', () => {
    ...
    const rate = new UIRate({ count: 5, value: 3, onChange: (value) => changes.push(value) });
  ...
  it('键盘 ←/→ 调整分值；disabled 不响应', () => {
    const rate = new UIRate({ count: 5, value: 2 });
...
describe('UISteps', () => {
  ...
  it('渲染标题与序号，当前步骤高亮', () => {
    const steps = new UISteps({ width: 400, items, current: 1 });
  ...
  it('setCurrent 夹取范围并更新节点状态', () => {
    const steps = new UISteps({ width: 400, items, current: 0 });
    steps.setCurrent(99);
...
describe('UIResult', () => {
  ...
  it('不同状态用不同图标与颜色', () => {
    const error = new UIResult({ width: 200, height: 160, status: 'error', title: '出错了' });
    expect(textsOf(error)).toContain('✕');
    const info = new UIResult({ width: 200, height: 160, status: 'info', title: '提示' });
