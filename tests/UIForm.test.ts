/**
 * 表单容器单测（UIFormItem / UIForm）。
 *
 * 规格：
 * - 控件实现统一取值约定（getFormValue / setFormValue），UIForm 直接读写控件；
...
function makeForm() {
  const form = new UIForm({ left: 0, top: 0, width: 320 });
  const nameField = new UITextField({ left: 0, top: 0, width: 200, height: 32 });
  const agreeBox = new UICheckBox({ left: 0, top: 0, width: 24, height: 24 });
  const nameItem = new UIFormItem({
    name: 'name',
    label: '姓名',
    control: nameField,
    rules: [{ required: true }],
  });
  const agreeItem = new UIFormItem({
    name: 'agree',
    label: '同意条款',
    control: agreeBox,
...
describe('UIFormItem', () => {
  ...
  it('纵向布局：标签在上、控件在中、错误文案在下', () => {
    const field = new UITextField({ width: 200, height: 32 });
    const item = new UIFormItem({ name: 'x', label: '标签', control: field, rules: [] });
...
describe('UIForm', () => {
  ...
  it('addItems 按纵向堆叠布局（gap 可配）', () => {
    const form = new UIForm({ left: 0, top: 0, width: 320, gap: 10 });
    const first = new UIFormItem({
      name: 'a',
      label: 'A',
      control: new UITextField({ width: 200, height: 32 }),
    });
    const second = new UIFormItem({
      name: 'b',
      label: 'B',
      control: new UITextField({ width: 200, height: 32 }),
    });
  ...
  it('外部传入 model 时直接复用（便于表单与业务共享状态）', () => {
    ...
    const form = new UIForm({ width: 320, model });
