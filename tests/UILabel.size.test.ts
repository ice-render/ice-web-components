 ...
 */
import { UIButton, UILabel, UIPanel, UIFlowLayout } from '../src';

function stubTextSize(label: UILabel, width: number, height: number): void {
...
describe('UILabel 尺寸', () => {
  ...
  it('调用方显式给了 width/height 时不被文字尺寸覆盖', () => {
    const label = new UILabel({ text: 'hello', width: 40, height: 20 });
  ...
  it('流式布局按标签宽度留位，后续组件不与其重叠', () => {
    const panel = new UIPanel({ width: 400, height: 60 });
    panel.setUILayout(new UIFlowLayout({ gap: 10 }));

    const label = new UILabel({ text: 'hello' });
    stubTextSize(label, 120, 20);
    const button = new UIButton({ text: 'Go', width: 80, height: 32 });
