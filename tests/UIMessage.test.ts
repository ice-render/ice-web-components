/**
 * UIMessage / UINotification 单测（消息堆叠 + 自动消失）。
 *
 * 规格：
 ...
 */
import { getUIMessageManager, UIMessageManager } from '../src/core/UIMessageManager';
...
describe('UIMessageManager', () => {
  ...
  it('show() 顶部居中堆叠，多条消息依次向下排', () => {
    const ice = makeICE();
    const manager = new UIMessageManager(ice).start();
  ...
  it('getUIMessageManager(ice) 按实例缓存', () => {
    const ice = makeICE();
    expect(getUIMessageManager(ice) === getUIMessageManager(ice)).toBe(true);
  });
});
