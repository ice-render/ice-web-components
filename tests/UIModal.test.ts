 ...
 * - 模态期间焦点被限制在对话框内（复用 UIFocusManager.setFocusScope），关闭后恢复原焦点；
...
function setup(options: any = {}) {
  const ice = makeICE();
  const overlays = new UIOverlayManager(ice);
  const focus = new UIFocusManager(ice).start();
  const modal = new UIModal(ice, {
    title: '标题',
    content: '正文',
    manager: overlays,
    focusManager: focus,
...
describe('UIModal', () => {
  ...
  it('遮罩覆盖全屏、可交互、且在场景组件之上（命中检测会优先命中它）', () => {
    const ice = makeICE();
    const sceneButton = new UIButton({ left: 100, top: 100, width: 120, height: 32, text: '下面' });
  ...
  it('焦点陷阱：打开后焦点在对话框内，关闭后恢复原焦点', () => {
    ...
    const outside = new UIButton({ left: 0, top: 0, width: 80, height: 32, text: '外部' });
