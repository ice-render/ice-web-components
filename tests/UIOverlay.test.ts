/**
 * 弹层底座（UIOverlayManager + resolveUIOverlayPosition）单测。
 *
...
import { resolveUIOverlayPosition } from '../src/util/UIOverlayPosition';
...
describe('resolveUIOverlayPosition', () => {
  ...
  it('bottom：水平居中对齐锚点，纵向留 offset', () => {
    const pos = resolveUIOverlayPosition({ ...base, placement: 'bottom' });
  ...
  it('bottomLeft / bottomRight：按锚点边对齐', () => {
    const left = resolveUIOverlayPosition({ ...base, placement: 'bottomLeft' });
    const right = resolveUIOverlayPosition({ ...base, placement: 'bottomRight' });
    expect(left.left).toBe(100);
    expect(right.left).toBe(60); // 180 - 120
  });

  it('right：纵向居中对齐，横向留 offset', () => {
    const pos = resolveUIOverlayPosition({ ...base, placement: 'right' });
    expect(pos.left).toBe(188); // 180 + 8
    expect(pos.top).toBe(95); // 115 - 20
  });

  it('rightTop / rightBottom：按锚点边对齐', () => {
    expect(resolveUIOverlayPosition({ ...base, placement: 'rightTop' }).top).toBe(100);
    expect(resolveUIOverlayPosition({ ...base, placement: 'rightBottom' }).top).toBe(90); // 130 - 40
  });
...
describe('UIOverlayManager', () => {
  it('open() 把内容挂到工具层的浮层根节点上，并按锚点定位', () => {
    ...
    const anchor = new UIComponent({ left: 100, top: 100, width: 80, height: 30 });
  ...
  it('锚点在深层容器里时按世界坐标定位', () => {
    const ice = makeICE();
    const manager = new UIOverlayManager(ice);
    const panelParent = new UIComponent({ left: 40, top: 60, width: 300, height: 200 });
    const anchor = new UIComponent({ left: 20, top: 10, width: 60, height: 20 });
  ...
  it('Esc 关闭浮层', () => {
    ...
    const handle = manager.open({
      anchor: new UIComponent({ left: 0, top: 0, width: 10, height: 10 }),
      content: panel(50, 20),
    });
