/**
 * 组件补全批次 2：Card extra 插槽 / Alert 图标 / Badge 计数封顶。
 */
import { UICard } from '../src/components/UICard';
import { UIAlert } from '../src/components/UIAlert';
import { UIBadge } from '../src/components/UIBadge';
import { UILabel } from '../src/components/UILabel';

describe('UICard extra 插槽', () => {
  it('工厂形式：节点创建在卡片之后（zIndex 高于卡片），右对齐到内边距', () => {
    const card = new UICard({
      title: 'T',
      width: 300,
      height: 160,
      extra: () => new UILabel({ text: 'more', width: 40, height: 16 }),
    });
    const extra = card.getExtraNode();
    expect(extra).not.toBeNull();
    expect(Number(extra!.state.zIndex)).toBeGreaterThan(Number(card.state.zIndex));
    expect(Number(extra!.state.left)).toBe(300 - 16 - 40);
  });

  it('传现成节点时也会被抬到卡片之上（避免被卡片底色盖住）', () => {
    const node = new UILabel({ text: 'x', width: 30, height: 14 });
    const card = new UICard({ title: 'T', width: 200, height: 100, extra: node });
    expect(card.getExtraNode()).toBe(node);
    expect(Number(node.state.zIndex)).toBeGreaterThan(Number(card.state.zIndex));
  });

  it('setExtra 替换；不传则没有 extra', () => {
    const card = new UICard({ title: 'T', width: 200, height: 100 });
    expect(card.getExtraNode()).toBeNull();
    const node = new UILabel({ text: 'y', width: 20, height: 14 });
    card.setExtra(node);
    expect(card.getExtraNode()).toBe(node);
    card.setExtra(null);
    expect(card.getExtraNode()).toBeNull();
  });
});

describe('UIAlert 图标', () => {
  it('默认显示类型图标，文案右移让位', () => {
    const withIcon = new UIAlert({ title: 't', message: 'm', type: 'success' });
    const icon = withIcon.getIconNode();
    expect(icon).not.toBeNull();
    expect(icon!.getText()).toBe('✓');

    const offset = Number(icon!.state.left) + Number(icon!.state.width);
    expect(Number(withIcon.getTitleNode().state.left)).toBeGreaterThanOrEqual(offset);
  });

  it('showIcon:false 不出图标，文案贴左边距', () => {
    const plain = new UIAlert({ title: 't', message: 'm', type: 'error', showIcon: false });
    expect(plain.getIconNode()).toBeNull();

    const themed = new UIAlert({ title: 't', type: 'error' });
    expect(themed.getIconNode()!.getText()).toBe('✕');
  });
});

describe('UIBadge 计数封顶', () => {
  it('count 转文案，超过 overflowCount 显示 N+', () => {
    expect(new UIBadge({ count: 5 }).getText()).toBe('5');
    expect(new UIBadge({ count: 128 }).getText()).toBe('99+');
    expect(new UIBadge({ count: 128, overflowCount: 999 }).getText()).toBe('128');
    expect(new UIBadge({ count: 0 }).getText()).toBe('0');
  });

  it('text 仍优先于 count；红点模式没有文字', () => {
    expect(new UIBadge({ text: 'new', count: 9 }).getText()).toBe('new');
    expect(new UIBadge({ dot: true, count: 9 }).getText()).toBe('');
  });
});
