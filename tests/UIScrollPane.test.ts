...
function makeRows(height = 100) {
  const content = new UIComponent({ left: 0, top: 0, width: 200, height });
  for (let i = 0; i < height / 20; i++) {
    content.addChild(new UILabel({ left: 0, top: i * 20, text: 'row ' + i }), false);
  }
  return content;
}

function attach(pane: UIScrollPane, ice: any) {
  ice.childNodes = [pane];
...
describe('UIScrollPane', () => {
  ...
  it('滚轮：指针在视口内且内容超出时滚动，视口外不响应', () => {
    const ice = makeICE();
    const pane = new UIScrollPane({ left: 100, top: 100, width: 200, height: 60 });
    attach(pane, ice);
