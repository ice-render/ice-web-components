/**
 * 选择模型单测（UIList / UITree / UITransfer 共用）。
 *
 * 语义：
 ...
 */
import { UISelectionModel } from '../src/model/UISelectionModel';

describe('UISelectionModel', () => {
  it('single 模式：选择替换，重复选择同一项不重复通知', () => {
    const model = new UISelectionModel({ mode: 'single' });
  ...
  it('multiple 模式：toggle 切换、select 追加', () => {
    const model = new UISelectionModel({ mode: 'multiple', selected: ['a'] });
    expect(model.getSelectedKeys()).toEqual(['a']);
    model.toggle('b');
  ...
  it('setSelected / clear / 注销监听器', () => {
    const model = new UISelectionModel({ mode: 'multiple' });
    let count = 0;
