 ...
 * - 点击行选中（走 UISelectionModel）、点击箭头只切换展开不改变选择；
 * - 键盘：↑/↓ 移动激活行、→ 展开/进入子节点、← 折叠/回父节点、Enter/Space 选中；
 * - 内容超出可视高度时套 UIScrollPane；onExpand / onSelect 回调。
 */
...
describe('UITree', () => {
  it('默认只显示根层；展开后子节点出现并缩进', () => {
    const tree = new UITree({ width: 240, height: 200, nodes });
  ...
  it('defaultExpandAll 全展开；箭头只切换展开、不改变选择', () => {
    const tree = new UITree({ width: 240, height: 240, nodes, defaultExpandAll: true, mode: 'single' });
  ...
  it('点击行选中（单选替换 / 多选切换），onSelect 回调', () => {
    ...
    const multiple = new UITree({ width: 240, height: 200, nodes, defaultExpandAll: true, mode: 'multiple' });
  ...
  it('内容超出高度时套 UIScrollPane；expandAll / collapseAll', () => {
    const tall = new UITree({ width: 240, height: 60, nodes, itemHeight: 30, defaultExpandAll: true });
